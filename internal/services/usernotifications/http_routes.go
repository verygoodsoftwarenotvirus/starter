...
func (s *service) CreateHandler(res http.ResponseWriter, req *http.Request) {
	...
	input := converters.ConvertUserNotificationCreationRequestInputToUserNotificationDatabaseCreationInput(providedInput)
	input.ID = identifiers.New()

	createTimer := timing.NewMetric("database").WithDesc("create").Start()
	userNotification, err := s.userNotificationDataManager.CreateUserNotification(ctx, input)
	if err != nil {
		observability.AcknowledgeError(err, logger, span, "creating user notification")
	...
	responseValue := &types.APIResponse[*types.UserNotification]{
...
func (s *service) ReadHandler(res http.ResponseWriter, req *http.Request) {
	...
	// fetch user notification from database.
	readTimer := timing.NewMetric("database").WithDesc("fetch").Start()
	x, err := s.userNotificationDataManager.GetUserNotification(ctx, sessionCtxData.Requester.UserID, userNotificationID)
	if errors.Is(err, sql.ErrNoRows) {
		errRes := types.NewAPIErrorResponse("not found", types.ErrDataNotFound, responseDetails)
		s.encoderDecoder.EncodeResponseWithStatus(ctx, res, errRes, http.StatusNotFound)
		return
	} else if err != nil {
		observability.AcknowledgeError(err, logger, span, "retrieving user notification")
		errRes := types.NewAPIErrorResponse("database error", types.ErrTalkingToDatabase, responseDetails)
		s.encoderDecoder.EncodeResponseWithStatus(ctx, res, errRes, http.StatusInternalServerError)
		return
	}
...
func (s *service) UpdateHandler(res http.ResponseWriter, req *http.Request) {
	...
	// fetch user notification from database.
	readTimer := timing.NewMetric("database").WithDesc("fetch").Start()
	userNotification, err := s.userNotificationDataManager.GetUserNotification(ctx, sessionCtxData.Requester.UserID, userNotificationID)
	if errors.Is(err, sql.ErrNoRows) {
		errRes := types.NewAPIErrorResponse("not found", types.ErrDataNotFound, responseDetails)
		s.encoderDecoder.EncodeResponseWithStatus(ctx, res, errRes, http.StatusNotFound)
		return
	} else if err != nil {
		observability.AcknowledgeError(err, logger, span, "retrieving user notification for update")
		errRes := types.NewAPIErrorResponse("database error", types.ErrTalkingToDatabase, responseDetails)
		s.encoderDecoder.EncodeResponseWithStatus(ctx, res, errRes, http.StatusInternalServerError)
		return
	}
	...
	userNotification.Update(input)

	updateTimer := timing.NewMetric("database").WithDesc("update").Start()
	if err = s.userNotificationDataManager.UpdateUserNotification(ctx, userNotification); err != nil {
		observability.AcknowledgeError(err, logger, span, "updating user notification")
		errRes := types.NewAPIErrorResponse("database error", types.ErrTalkingToDatabase, responseDetails)
		s.encoderDecoder.EncodeResponseWithStatus(ctx, res, errRes, http.StatusInternalServerError)
		return
	}
